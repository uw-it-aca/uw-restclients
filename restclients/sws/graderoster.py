...
from restclients.models.sws import GradeRoster as GradeRosterModel
from restclients.models.sws import GradeRosterItem, GradeSubmissionDelegate
from lxml import etree
import re


class GradeRoster(SWS):
    def get(self, section, instructor):
        """
        Returns a restclients.GradeRoster for the passed Section model
        and instructor Person.
        """
        url = "/student/v4/graderoster/%s" % self._label(section, instructor)
        ...
        return self.graderoster_from_xhtml(response.data, section, instructor)

    def update(self, graderoster):
        """
    ...
    def _label(self, section, instructor):
        label = section.section_label()
        label = re.sub(r"/", ",", label)
        label = re.sub(r"\s", "%20", label)
        label += "," + instructor.uwregid
        return label

    def graderoster_from_xhtml(self, data, section, instructor):
        pws = PWS()
        people = {instructor.uwregid: instructor}

        graderoster = GradeRosterModel()
